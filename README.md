...
1. 在「除蟲藥」卡片按「啟用推播」，瀏覽器會請你同意通知權限，同意後會把訂閱資訊存進 `push_subscriptions`。
2. 每天點除蟲藥時，用卡片右上角「+」新增一筆紀錄。
3. Vercel 排程（`vercel.json` 裡的 Cron Job）每天呼叫一次 `/api/cron/flea-reminder`：算出「上次點藥日期 + 3 個月」，如果已經到期、而且這次到期還沒推播過，就對所有已訂閱的裝置發送推播通知，同時清掉已經失效的訂閱。
...
之後每次 push 到部署分支，Vercel 都會自動重新部署；`vercel.json` 裡設定的 Cron Job 也會跟著自動排程（預設每天台灣時間早上 9 點檢查一次）。

## 專案結構

```
app/                        # Next.js App Router 頁面與 API 路由
app/api/cron/flea-reminder  # Vercel Cron 呼叫的除蟲藥提醒檢查
components/                 # 前端元件（寵物資訊卡、除蟲藥提醒卡、今日紀錄格、新增紀錄彈窗、清單、日期切換）
lib/                        # Supabase client、型別定義、日期工具、頭像上傳、推播訂閱
public/sw.js                # 接收推播通知的 Service Worker
supabase/schema.sql         # entries / push_subscriptions / reminder_state 資料表結構
supabase/storage.sql        # 頭像照片 storage bucket 與權限
vercel.json                 # 除蟲藥提醒的 Cron Job 排程設定
```
